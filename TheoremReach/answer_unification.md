...
The main retry loop (Phase 3.B orphan handling and Phase 3.C intra-cluster audit) uses **threshold-based termination** to balance convergence quality against runtime/cost.

> [!NOTE]
> **Phase 3.C (Intra-Cluster Audit)**: Before the retry loop exits, the system runs a lightweight intra-cluster audit (using `intra_cluster_only=True`) to detect transitive bridges. If any answers are evicted, the loop retries immediately—ensuring cluster stability is validated in the same run rather than waiting for Phase 4.
...
flowchart TD
    ...
    subgraph Phase4["Phase 4: Entailment & Audit"]
        MATCH -->|Filter| REPS[Cluster Representatives]
        REPS -->|Vector Search| OV_CAND[Overlap & Intra-Cluster Candidates]
        OV_CAND -->|LLM Check| OV_LLM{Does X entail Y?}
        
        OV_LLM -->|YES (Inter)| OVERLAPS[overlaps.json]
        OV_LLM -->|NO (Intra)| EVICT[Evict Transitive Bridge]
...
### Phase 4: Entailment & Intra-Cluster Audit

Phase 4 performs two critical semantic checks using "Strict Entailment" reasoning ("Does A guarantee B?").

#### 1. Multi-Cluster Membership (Entailment)
Allows users to qualify for broader targets based on specific answers.
- **Goal**: If a user answers "Full-time/Part-time" (Source), they logically entail "Full-time" (Target).
- **Candidates**: Generated by comparing Cluster Representatives via vector similarity (Inter-Cluster).
- **Output**: `overlap_map` (JSON) maps Source ID -> List of Broader IDs.
- **Consumption**: `AnswerUnificationWorker` persists this as `fv2c` in Redis, enabling the user to match multiple clusters in O(1) time.

#### 2. Intra-Cluster Audit (Transitive Bridge Protection)
Detects and repairs "Transitive Bridges" where valid pairwise links form an invalid chain (e.g., `Associate's <-> University Degree <-> Bachelor's`).
- **Goal**: Ensure every member of a cluster is strictly equivalent to the **Cluster Representative**.
- **Logic**: Performs a **Bidirectional Entailment Check**:
    1.  **Member -> Rep**: Does Member imply Rep?
    2.  **Rep -> Member**: Does Rep imply Member?
- **Eviction**: If *either* check fails, the member is **EVICTED** from the cluster.
- **Result**: The "bridge" answer (University Degree) is removed, breaking the false link. The distinct sub-groups (Associate's, Bachelor's) then naturally split into separate clusters in the next run.

#### Phase 4 Pipeline:
1.  **Representatives**: An **Answer Representative** is elected for each cluster (Flagship Priority applies).
2.  **Candidate Generation**:
    - **Inter-Cluster**: Rep vs Rep vector search (for Entailment).
    - **Intra-Cluster**: Member vs Rep (for Audit).
3.  **LLM Verification**: Asks: "Does a user with answer '{Source}' GUARANTEES qualification for '{Target}'?"
4.  **Batching**: Decisions are processed in async batches (default size 50) and cached in SQLite.
